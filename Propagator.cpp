...
Propagator::Derive Propagator::derivatives(const Satellite &sat)
{
...
{
	Derive k1 = derivatives(sat); 
	Satellite s2(sat.Velocity.add(k1.dv.multiply(dt * 0.5)),
			sat.Position.add(k1.dr.multiply(dt * 0.5))
			); 
	
	Derive k2 = derivatives(s2); 
	Satellite s3(sat.Velocity.add(k2.dv.multiply(dt * 0.5)),
			sat.Position.add(k2.dr.multiply(dt * 0.5))
			); 
	Derive k3 = derivatives(s3); 
	Satellite s4(sat.Velocity.add(k3.dv.multiply(dt * 0.5)),
			sat.Position.add(k3.dr.multiply(dt * 0.5))
			); 
	Derive k4 = derivatives(s4); 
